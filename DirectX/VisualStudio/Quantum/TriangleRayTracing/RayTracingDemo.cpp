...
mBlueColor(1.0f), mPosition(0, 0, -5), mLightPosition(0, 0, -5), mCamera(&camera)//mCamera(&camera)
{
}
...
void RayTracingDemo::initialize()
{
	...
	mEffect->LoadCompiledEffect(L"TriangleRayTraceEffect.cso");
	...
	{
		XMFLOAT3 position(getRandomFloat(MIN_POSITION, MAX_POSITION), getRandomFloat(MIN_POSITION, MAX_POSITION), getRandomFloat(-200.0f, 0));
		spheres[i] = RayTraceSphere(*mGame, PinholeCamera(*mGame), position);
		spheres[i].setColor(getRandomFloat(0.0f, 1.0f), getRandomFloat(0.0f, 1.0f), getRandomFloat(0.0f, 1.0f));

	}

	mesh = new TriMesh(*mGame, std::string("Content/Models/basicCube.obj"));
	//mCamera->setViewDistance(1000.0f);
	//mCamera->setEyeVector(0, 0, 5);
...
void RayTracingDemo::update(const GameTimer & timer)
{
	...
	if (mKeyboard != nullptr)
	{
		...
		XMVECTOR position = XMLoadFloat3(&mPosition) + (XMLoadFloat3(&movementAmount) * 10.0f* timer.elapsedGameTime());
	...
	if (mKeyboard != nullptr)
	{
		...
		XMVECTOR lightPosition = XMLoadFloat3(&mLightPosition) + (XMLoadFloat3(&lightMovementAmount) * 10.0f* timer.elapsedGameTime());
...
void RayTracingDemo::draw(const GameTimer & timer)
{
	...
	//mMaterial->CameraPosition() << mCamera->eyeVector();
	mMaterial->LightPosition() << XMLoadFloat3(&mLightPosition);
	mMaterial->Color() << spheres[0].colorVector();
	...
	//mMaterial->CameraDirection() << XMVector3Normalize(mCamera->eyeVector() - mCamera->lookAtVector());
	...
	for (size_t i = 0; i <mesh->mTriangles.size(); i++)
	{
		...
		mMaterial->v0() <<  XMLoadFloat3(&mesh->mTriangles[i].v0);
		mMaterial->v1() <<  XMLoadFloat3(&mesh->mTriangles[i].v1);
		mMaterial->v2() <<  XMLoadFloat3(&mesh->mTriangles[i].v2);
		mMaterial->Normal() << XMLoadFloat3(&mesh->mTriangles[i].normal);
