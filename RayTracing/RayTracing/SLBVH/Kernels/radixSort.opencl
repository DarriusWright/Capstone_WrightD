...
__kernel void computeHistogram(__global const MortonCode* data,
    ...
    uint result= (data[globalId].code >> shiftBy) & 0xFFU;
...
__kernel void rankNPermute(__global const MortonCode* unsortedData,
                           __global const uint* scannedHistogram,
                           uint shiftCount,
                           __local ushort* sharedBuckets,
                           __global MortonCode* sortedData) { 
    ...
    for(int i = 0; i < R; ++i)
    {
        uint value = unsortedData[gidx * R + i].code;
		uint uIndex0 = unsortedData[gidx * R + i].index;

        value = (value >> shiftCount) & 0xFFU;
        uint index = sharedBuckets[idx * R + value];
        sortedData[index].code = unsortedData[gidx * R + i].code;
        sortedData[index].index = uIndex0;
