...
void RenderWindow::construct()
{
	...
	for(int i = 0; i < NUMBER_OF_SPHERES; i++)
	{
		...
		{random.getRandomFloat(0.0,1.0),random.getRandomFloat(0.0,1.0),random.getRandomFloat(0.0,1.0)}},
		{ random.getRandomFloat(-100.0f,100.0f), random.getRandomFloat(-100.0f,100.0f), random.getRandomFloat(30.0f,50.0f),30.0f}};
		...
		box.min[0] = glm::min((sphere.getMinX() + sphere.position.s[0]) ,box.min[0]);
		box.min[1] = glm::min(sphere.getMinY()  +  sphere.position.s[1],box.min[1]);
		box.min[2] = glm::min(sphere.getMinZ()	 + sphere.position.s[2],box.min[2]);

		box.max[0] = glm::max(sphere.getMaxX()+ sphere.position.s[0],box.max[0]);
		box.max[1] = glm::max(sphere.getMaxY()+  sphere.position.s[1],box.max[1]);
		box.max[2] = glm::max(sphere.getMaxZ()+ sphere.position.s[2],box.max[2]);
